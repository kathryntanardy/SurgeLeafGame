<script lang="ts">
	...
	import { Stock, scoreStore } from '$lib/game/LeafGame';
					...
					<div class="card">
						<img
							src={p?.state === Stock.OutOfStock
								? `/shop_restock/${fileByKey[key]}`
								: p?.state === Stock.Default
									? `/shop_unlock/${fileByKey[key]}`
									: `/shop_restock/${fileByKey[key]}`}
							alt={key}
							class="plant-img"
							draggable="false"
...
<style>
	...
	.restock-btn {
		background-image: url('/shop_restock/button.png');
	}
	.unlock-btn {
		background-image: url('/shop_unlock/button.png');
	}
	.action-btn:disabled {
		opacity: 0.5;
		cursor: default;
